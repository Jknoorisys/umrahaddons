...
class Payment extends ResourceController
{
	...
	public function __construct()
	{
		...
		if (!$this->service->getAccessForSignedUser($token)) {
	...
	public function successPayment()
	{
		...
		$admin_commision_per = $providerdata['commision_percent'];
		...
		// ota  commission
		$ota_commision = $otadata['commision_percent'];
		...
		if ($status == 'success') {
			...
			if ($BookingModel->insert($inprocessbooking)) {
				...
				if ($confirm_payment_status == 'paid') {
					$confirm_booking = [
	...
	public function failedPayment()
	{
		...
		$user_pax = $this->request->getPost('user_pax');
		$status = $this->request->getPost('status');
		$payment_intent = $this->request->getPost('payment_intent');
		...
		// ota  commission
		$ota_commision = $otadata['commision_percent'];
		// echo json_encode($ota_commision);die();
		$ota_precent = $ota_commision / 100;
		$ota_ammount = $ota_precent * $rate;
