...
class ManageCities extends BaseController
{
    ...
    public function cityList()
    {
        ...
        try{
            ...
            // By Query Builder
            $db = db_connect();
            $citysData = $db->table('tbl_city_master as s')
            ...
            $total =  $db->table('tbl_city_master as s')->where($whereCondition)->countAllResults();
    ...
    public function addCity()
    {
        ...
        $rules = [
            ...
            'name' => [
                'rules'         =>  'required|is_unique[tbl_city_master.name]',
                'errors'        => [
                    'required'      =>  Lang('Language.required'),
        ...
        try {
            ...
            $db = db_connect();
            $insert = $db->table('tbl_city_master')->insert($data);
            if($insert) 
            {
                return $service->success([
    ...
    public function editCity()
    {
        ...
        try {
            ...
            if ($this->request->getFile('image')) {
                ...
                $photo_url = $cityDetails['image'];
            }

            $data = [
                'name'          =>    $name ? $name : $cityDetails['name'],
                'image'         =>    $photo_url ? $photo_url : $cityDetails['image'],
                'updated_at'    => date('Y-m-d H:i:s'),
            ];

            $db = db_connect();
            $update = $db->table('tbl_city_master')
                ->where('id', $city_id)
                ->update($data);
    ...
    public function deleteCity()
    {
        ...
        try {
            ...
            $db = db_connect();
            $delete = $db->table('tbl_city_master')
                ->where('id', $city_id)
                ->set('status', '2')
                ->update();
